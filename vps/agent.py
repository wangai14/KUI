...
def get_system_status():
    ...
    try:
        ...
        if total_match and avail_match:
            total = float(total_match.group(1)); avail = float(avail_match.group(1))
            stats["mem"] = round(((total - avail) / total) * 100, 2)
    except Exception: pass

    try:
        st = os.statvfs('/')
        total_disk = st.f_blocks * st.f_frsize; used_disk = (st.f_blocks - st.f_bfree) * st.f_frsize
        if total_disk > 0: stats["disk"] = int((used_disk / total_disk) * 100)
    except Exception: pass

    try:
        with open('/proc/uptime', 'r') as f:
            uptime_seconds = float(f.readline().split()[0])
            days = int(uptime_seconds // 86400); hours = int((uptime_seconds % 86400) // 3600); mins = int((uptime_seconds % 3600) // 60)
...
def report_status(current_nodes, argo_urls):
    ...
    status["ip"] = VPS_IP; status["argo_urls"] = argo_urls
...
def build_singbox_config(nodes):
    ...
    for node in nodes:
        in_tag = f"in-{node['id']}"; proto = node["protocol"]
        ...
        if proto == "VLESS":
            ...
            # 🌟 优化：移除外层 tls.server_name 防止严格校验阻断，交由 reality 处理
            singbox_config["inbounds"].append({
                "type": "vless", "tag": in_tag, "listen": "::", "listen_port": int(node["port"]),
